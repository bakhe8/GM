...
namespace GuaranteeManager.Services
{
    internal class GuaranteeRepository
    {
        ...
        public int UpdateGuaranteeWithAttachments(Guarantee g, List<AttachmentInput> newAttachments, List<AttachmentRecord> removedAttachments)
        {
            ...
            ValidateAdministrativeUpdate(g);
            ...
            List<AttachmentRecord> effectiveRemovedAttachments = removedAttachments ?? new List<AttachmentRecord>();
            var inheritedAttachments = g.Attachments
                .Where(a => !effectiveRemovedAttachments.Any(r => r.Id == a.Id))
                .ToList();

            int newId = CreateNewVersionWithAttachments(g, g.Id, newAttachments, inheritedAttachments);
            SimpleLogger.Log($"UpdateGuarantee: created version for {g.GuaranteeNo} (oldId={g.Id}, newId={newId}, removedCount={effectiveRemovedAttachments.Count}).");
        ...
        {
            using var scope = SimpleLogger.BeginScope("GuaranteeRepository.SumGuaranteeAmounts");
            GuaranteeQueryOptions effectiveOptions = options ?? new GuaranteeQueryOptions();

            try
            {
                using var connection = SqliteConnectionFactory.Open(_connectionString);
                using SqliteCommand cmd = BuildGuaranteeAmountSumCommand(connection, effectiveOptions);
            ...
            catch (Exception ex)
            {
                throw OperationFailure.LogAndWrap(
                    ex,
                    "GuaranteeRepository.SumGuaranteeAmounts",
                    "تعذر احتساب إجمالي مبالغ الضمانات الحالية.");
            }
        }

        public List<BankPortfolioSummary> GetBankPortfolioSummaries()
        {
            using var scope = SimpleLogger.BeginScope("GuaranteeRepository.GetBankPortfolioSummaries");
            var summaries = new List<BankPortfolioSummary>();
            ...
            try
            {
                ...
                command.CommandText = @"
                    WITH CurrentGuarantees AS (
                        SELECT
                            TRIM(IFNULL(Bank, '')) AS Bank,
                        ...
                        FROM Guarantees
                        WHERE IsCurrent = 1
                          AND TRIM(IFNULL(Bank, '')) <> ''
                    ),
                    BankTotals AS (
                        SELECT
                            LOWER(Bank) AS BankKey,
                            MIN(Bank) AS Bank,
                            COUNT(*) AS GuaranteeCount,
                            SUM(CASE WHEN LifecycleStatus = 'Active' THEN 1 ELSE 0 END) AS ActiveCount,
                            SUM(CASE WHEN date(ExpiryDate) >= date('now')
                                      AND date(ExpiryDate) <= date('now', '+30 days') THEN 1 ELSE 0 END) AS ExpiringSoonCount,
                            SUM(CASE WHEN date(ExpiryDate) < date('now') THEN 1 ELSE 0 END) AS ExpiredCount,
                            COALESCE(SUM(Amount), 0) AS TotalAmount
                        FROM CurrentGuarantees
                        GROUP BY LOWER(Bank)
                    ),
                    SupplierCounts AS (
                        SELECT
                            LOWER(Bank) AS BankKey,
                            CASE WHEN Supplier = '' THEN '---' ELSE Supplier END AS Supplier,
                            COUNT(*) AS SupplierCount
                        FROM CurrentGuarantees
                        GROUP BY LOWER(Bank), CASE WHEN Supplier = '' THEN '---' ELSE Supplier END
                    ),
                    RankedSuppliers AS (
                        SELECT
                            BankKey,
                            Supplier,
                            ROW_NUMBER() OVER (
                                PARTITION BY BankKey
                                ORDER BY SupplierCount DESC, Supplier ASC
                            ) AS RowNumber
                        FROM SupplierCounts
                    )
                    SELECT
                        BankTotals.Bank,
                        BankTotals.GuaranteeCount,
                        BankTotals.ActiveCount,
                        BankTotals.ExpiringSoonCount,
                        BankTotals.ExpiredCount,
                        BankTotals.TotalAmount,
                        COALESCE(RankedSuppliers.Supplier, '---') AS TopSupplier
                    FROM BankTotals
                    LEFT JOIN RankedSuppliers
                        ON RankedSuppliers.BankKey = BankTotals.BankKey
                       AND RankedSuppliers.RowNumber = 1
                    ORDER BY BankTotals.TotalAmount DESC, BankTotals.Bank ASC";

                using SqliteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    summaries.Add(new BankPortfolioSummary(
                        reader.GetString(0),
            ...
            catch (Exception ex)
            {
                throw OperationFailure.LogAndWrap(
                    ex,
                    "GuaranteeRepository.GetBankPortfolioSummaries",
                    "تعذر تحميل ملخصات البنوك الحالية.");
            }
        ...
        private void ValidateAdministrativeUpdate(Guarantee incoming)
        {
            if (incoming == null)
            {
            ...
            int rootId = incoming.RootId ?? incoming.Id;
            Guarantee current = GetCurrentGuaranteeByRootId(rootId, connection)
                ?? throw new InvalidOperationException("تعذر تحميل النسخة الحالية للضمان قبل التعديل.");

            if (current.Id != incoming.Id)
            {
                throw new InvalidOperationException("لا يمكن تعديل نسخة قديمة من الضمان. أعد تحميل النسخة الحالية ثم حاول مرة أخرى.");
            }

            List<string> changedOperationalFields = GetChangedOperationalFields(current, incoming);
            if (changedOperationalFields.Count > 0)
            {
                throw new OperationalFieldMutationException(changedOperationalFields);
            }
        }

        private static List<string> GetChangedOperationalFields(Guarantee current, Guarantee incoming)
        {
            var changedFields = new List<string>();

            if (!GuaranteeDataAccess.GuaranteeNumbersEqual(incoming.GuaranteeNo, current.GuaranteeNo))
            {
                changedFields.Add("رقم الضمان");
            }

            if (!SameText(incoming.Bank, current.Bank))
            {
                changedFields.Add("البنك");
            }

            if (!SameText(incoming.GuaranteeType, current.GuaranteeType))
            {
                changedFields.Add("نوع الضمان");
            }

            if (incoming.Amount != current.Amount)
            {
                changedFields.Add("المبلغ");
            }

            if (incoming.ExpiryDate.Date != current.ExpiryDate.Date)
            {
                changedFields.Add("تاريخ الانتهاء");
            }

            if (incoming.DateCalendar != current.DateCalendar)
            {
                changedFields.Add("تقويم التاريخ");
            }

            (GuaranteeReferenceType currentReferenceType, string currentReferenceNumber) =
                GuaranteeDataAccess.NormalizeReference(current.ReferenceType, current.ReferenceNumber);
            (GuaranteeReferenceType incomingReferenceType, string incomingReferenceNumber) =
                GuaranteeDataAccess.NormalizeReference(incoming.ReferenceType, incoming.ReferenceNumber);

            if (incomingReferenceType != currentReferenceType)
            {
                changedFields.Add("نوع المرجع");
            }

            if (!SameText(incomingReferenceNumber, currentReferenceNumber))
            {
                changedFields.Add("رقم المرجع");
            }

            if (incoming.LifecycleStatus != current.LifecycleStatus)
            {
                changedFields.Add("الحالة التشغيلية");
            }

            if (incoming.ReplacesRootId != current.ReplacesRootId)
            {
                changedFields.Add("رابط الضمان المستبدل");
            }

            if (incoming.ReplacedByRootId != current.ReplacedByRootId)
            {
                changedFields.Add("رابط الضمان البديل");
            }
        ...
        private static SqliteCommand BuildGuaranteeAmountSumCommand(SqliteConnection connection, GuaranteeQueryOptions options)
        {
            StringBuilder sql = new("SELECT COALESCE(SUM(Amount), 0) FROM Guarantees");
            SqliteCommand command = connection.CreateCommand();
            AppendGuaranteeFilters(sql, command, options);
            command.CommandText = sql.ToString();
            return command;
 ...
 AND (
     ...
     OR LOWER(IFNULL(Beneficiary, '')) LIKE $search
            ...
            AppendRootIdSetFilter(sql, command, options.IncludeRootIds, "includeRoot", include: true);
            AppendRootIdSetFilter(sql, command, options.ExcludeRootIds, "excludeRoot", include: false);
            ...
            {
                List<int> followUpPendingRootIds = NormalizeRootIds(options.FollowUpPendingRootIds);
                if (followUpPendingRootIds.Count > 0)
                {
                    List<string> followUpRootParameters = AddRootIdParameters(command, "followUpRoot", followUpPendingRootIds);
                    sql.Append($@"
 AND (
        (ExpiryDate < $todayDate AND LifecycleStatus IN ($expiryActiveStatus, $expiryExpiredStatus))
     OR (
            LifecycleStatus = $followUpActiveStatus
        AND ExpiryDate >= $todayDate
        AND ExpiryDate <= $expiringSoonCutoff
        AND COALESCE(RootId, Id) IN ({string.Join(", ", followUpRootParameters)})
        )
 )");
                    command.Parameters.AddWithValue("$followUpActiveStatus", GuaranteeLifecycleStatus.Active.ToString());
                }
                else
                {
        ...
        private static void AppendRootIdSetFilter(
            StringBuilder sql,
            ...
            bool include)
        {
            ...
            List<int> normalizedRootIds = NormalizeRootIds(rootIds);
            if (normalizedRootIds.Count == 0)
            {
                if (include)
                {
                    sql.Append(" AND 1 = 0");
            ...
            List<string> parameterNames = AddRootIdParameters(command, parameterPrefix, normalizedRootIds);
            sql.Append($" AND COALESCE(RootId, Id) {(include ? "IN" : "NOT IN")} ({string.Join(", ", parameterNames)})");
        }

        private static List<int> NormalizeRootIds(IReadOnlyCollection<int>? rootIds)
            => (rootIds ?? Array.Empty<int>())
                .Where(rootId => rootId > 0)
                .Distinct()
                .ToList();

        private static List<string> AddRootIdParameters(
            SqliteCommand command,
            string parameterPrefix,
            IReadOnlyCollection<int> rootIds)
        {
            ...
            {
                string parameterName = $"${parameterPrefix}{index.ToString(CultureInfo.InvariantCulture)}";
                parameterNames.Add(parameterName);
                command.Parameters.AddWithValue(parameterName, rootId);
                index++;
            }

...
namespace GuaranteeManager.Services
{
    internal class WorkflowRequestRepository
    {
        ...
        public int SaveWorkflowRequest(WorkflowRequest req)
        {
            ...
            try
            {
                if (req.Status == RequestStatus.Pending
                    && HasPendingWorkflowRequest(req.RootGuaranteeId, req.Type, connection, transaction))
                {
                    throw new InvalidOperationException(
                        "يوجد بالفعل طلب معلق من نفس النوع لهذا الضمان. افتح الطلب الحالي أو سجل رد البنك قبل إنشاء طلب جديد.");
                }
        ...
        public bool HasPendingWorkflowRequest(int rootId, RequestType requestType)
        {
            ...
            {
                using var connection = SqliteConnectionFactory.Open(_connectionString);
                return HasPendingWorkflowRequest(rootId, requestType, connection);
            }
            catch (Exception ex)
            {
                throw OperationFailure.LogAndWrap(
        ...
        private static bool HasPendingWorkflowRequest(
            int rootId,
        ...
        {
            using var scope = SimpleLogger.BeginScope("WorkflowRequestRepository.GetPendingWorkflowRequestRootIds");
            ...
            try
            {
                ...
                command.CommandText = @"
                    ...
                    FROM WorkflowRequests
                    WHERE RequestStatus = $pendingStatus
                    ORDER BY RootId";
            ...
            catch (Exception ex)
            {
                throw OperationFailure.LogAndWrap(
                    ex,
                    "WorkflowRequestRepository.GetPendingWorkflowRequestRootIds",
                    "تعذر تحميل الضمانات المرتبطة بطلبات معلقة.");
            }
        ...
        {
            using var scope = SimpleLogger.BeginScope("WorkflowRequestRepository.GetWorkflowRequestsByRootIds");
            List<int> distinctRootIds = NormalizeRootIds(rootIds);
            Dictionary<int, List<WorkflowRequest>> result = distinctRootIds
                .ToDictionary(rootId => rootId, _ => new List<WorkflowRequest>());
            ...
            {
                using var connection = SqliteConnectionFactory.Open(_connectionString);
                Dictionary<int, GuaranteeDateCalendar> dateCalendars = GetCurrentDateCalendarsByRootIds(distinctRootIds, connection);
                using SqliteCommand command = connection.CreateCommand();
                List<string> parameterNames = AddRootIdParameters(command, "root", distinctRootIds);
                command.CommandText = $@"
                    SELECT {WorkflowRequestDataAccess.SelectColumns}
                    FROM WorkflowRequests
                    WHERE RootId IN ({string.Join(", ", parameterNames)})
                    ORDER BY RootId ASC, SequenceNumber DESC, RequestDate DESC";

                using SqliteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    WorkflowRequest request = WorkflowRequestDataAccess.MapWorkflowRequest(reader);
                    request.DateCalendar = dateCalendars.TryGetValue(request.RootGuaranteeId, out GuaranteeDateCalendar calendar)
                        ? calendar
                        : GuaranteeDateCalendar.Gregorian;

                    if (!result.TryGetValue(request.RootGuaranteeId, out List<WorkflowRequest>? bucket))
                    {
                        bucket = new List<WorkflowRequest>();
                        result[request.RootGuaranteeId] = bucket;
                    }
            ...
            catch (Exception ex)
            {
                throw OperationFailure.LogAndWrap(
                    ex,
                    "WorkflowRequestRepository.GetWorkflowRequestsByRootIds",
                    "تعذر تحميل تسلسل الطلبات للضمانات المعروضة.");
            }
        }
            ...
            string responseSavedFileName)
        {
            ...
            try
            {
                ...
                cmd.CommandText = @"
                    UPDATE WorkflowRequests SET
                        UpdatedAt = $now,
                        ResponseRecordedAt = COALESCE(ResponseRecordedAt, $now),
                        ResponseNotes = $responseNotes,
                        ResponseOriginalFileName = $responseOriginalFileName,
                        ResponseSavedFileName = $responseSavedFileName
                    WHERE Id = $id";
        ...
        private static void AppendWorkflowRequestFilters(StringBuilder sql, SqliteCommand command, WorkflowRequestQueryOptions options)
        {
            ...
            if (options.RootGuaranteeIds is { Count: > 0 } rootGuaranteeIds)
            {
                List<int> distinctRootIds = rootGuaranteeIds
                ...
                else
                {
                    ...
                    {
                        string parameterName = $"$rootGuaranteeId{index}";
                        parameterNames.Add(parameterName);
                        command.Parameters.AddWithValue(parameterName, distinctRootIds[index]);
                    }

                    sql.Append($" AND wr.RootId IN ({string.Join(", ", parameterNames)})");
                }
            }
 ...
 AND (
     ...
     OR LOWER(IFNULL(wr.CreatedBy, '')) LIKE $search");
        ...
        private static Dictionary<int, GuaranteeDateCalendar> GetCurrentDateCalendarsByRootIds(
            IReadOnlyCollection<int> rootIds,
            SqliteConnection connection,
            SqliteTransaction? transaction = null)
        {
            List<int> distinctRootIds = NormalizeRootIds(rootIds);
            var result = new Dictionary<int, GuaranteeDateCalendar>();
            if (distinctRootIds.Count == 0)
            {
                return result;
            }
            ...
            using SqliteCommand command = connection.CreateCommand();
            command.Transaction = transaction;
            List<string> parameterNames = AddRootIdParameters(command, "calendarRoot", distinctRootIds);
            command.CommandText = $@"
                SELECT COALESCE(RootId, Id) AS RootId, DateCalendar
                FROM Guarantees
                WHERE IsCurrent = 1
                  AND COALESCE(RootId, Id) IN ({string.Join(", ", parameterNames)})";

            using SqliteDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                int rootId = reader.GetInt32(0);
                result[rootId] = reader.IsDBNull(1)
                    ? GuaranteeDateCalendar.Gregorian
                    : DualCalendarDateService.ParseDateCalendar(reader.GetString(1));
            }
            ...
            IReadOnlyCollection<int> rootIds)
        {
            ...
            {
                string parameterName = $"${parameterPrefix}{index.ToString(System.Globalization.CultureInfo.InvariantCulture)}";
                parameterNames.Add(parameterName);

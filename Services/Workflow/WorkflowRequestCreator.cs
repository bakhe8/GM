...
namespace GuaranteeManager.Services
{
    internal sealed class WorkflowRequestCreator
    {
        ...
        public WorkflowRequestCreator(IDatabaseService databaseService, WorkflowLetterService letterService)
            : this(databaseService, letterService, new AttachmentStorageService())
        {
        }

        internal WorkflowRequestCreator(
        ...
        public WorkflowRequest CreateReplacementRequest(
            ...
            string notes,
            string createdBy = "",
            IReadOnlyList<AttachmentInput>? replacementAttachments = null)
        {
            Guarantee guarantee = ResolveCurrentGuarantee(guaranteeId);
            int rootId = guarantee.RootId ?? guarantee.Id;
            string normalizedCreatedBy = WorkflowCreatedByPolicy.NormalizeForNewRequest(createdBy);
            ...
            List<AttachmentInput> attachmentInputs = (replacementAttachments ?? Array.Empty<AttachmentInput>())
                .Where(attachment => !string.IsNullOrWhiteSpace(attachment.FilePath))
                .ToList();
            List<StagedAttachmentFile> stagedReplacementAttachments = _attachmentStorage.StageCopies(
                attachmentInputs.Select(attachment => attachment.FilePath));

            try
            {
                _attachmentStorage.FinalizeStagedCopies(stagedReplacementAttachments, "CreateReplacementRequestAttachments");
                ...
                return SaveLetterBackedRequest(
                    ...
                    new WorkflowRequestedData
                    {
                        ...
                        ReplacementAttachments = BuildReplacementAttachmentData(attachmentInputs, stagedReplacementAttachments)
                    },
                    guarantee.DateCalendar,
                    generatedLetter,
                    notes,
            ...
            {
                _attachmentStorage.CleanupStagedCopies(stagedReplacementAttachments);
                foreach (StagedAttachmentFile stagedAttachment in stagedReplacementAttachments)
                {
                    TryDeleteFile(stagedAttachment.FinalPath);
                }
        ...
        private static List<WorkflowRequestAttachmentData> BuildReplacementAttachmentData(
            IReadOnlyList<AttachmentInput> attachmentInputs,
            IReadOnlyList<StagedAttachmentFile> stagedAttachments)
        {
            var result = new List<WorkflowRequestAttachmentData>();
            for (int index = 0; index < stagedAttachments.Count; index++)
            {
                StagedAttachmentFile staged = stagedAttachments[index];
                AttachmentDocumentType documentType = index < attachmentInputs.Count
                    ? attachmentInputs[index].DocumentType
                    : AttachmentDocumentType.GuaranteeImage;

                result.Add(new WorkflowRequestAttachmentData
                {
                    OriginalFileName = staged.OriginalFileName,
                    SavedFileName = staged.SavedFileName,
        ...
        private static void TryDeleteFile(string path)
        {
            ...
            {
                PendingFileOperationQueue.RecordCleanupFailure(Path.GetFileName(path), path, ex);

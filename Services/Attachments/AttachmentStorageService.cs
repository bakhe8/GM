...
namespace GuaranteeManager.Services
{
    ...
    {
        public const long MaxAttachmentFileSizeBytes = 25L * 1024 * 1024;

        public virtual List<StagedAttachmentFile> StageCopies(IEnumerable<string> sourcePaths)
        {
            ...
            try
            {
                ...
                {
                    EnsureSupportedFileSize(sourcePath);
        ...
        public virtual void CleanupStagedCopies(IEnumerable<StagedAttachmentFile> stagedCopies)
        {
            foreach (var stagedCopy in stagedCopies)
            {
                ...
                catch (Exception cleanupEx)
                {
                    PendingFileOperationQueue.RecordCleanupFailure(
                        stagedCopy.SavedFileName,
                        stagedCopy.StagingPath,
                        cleanupEx);
                    SimpleLogger.Log(
        ...
        public virtual void FinalizeStagedCopies(IEnumerable<StagedAttachmentFile> stagedCopies, string operationName)
        {
            ...
            foreach (var stagedCopy in stagedCopies)
            {
                ...
                {
                    PendingFileOperationQueue.RecordAttachmentPromotionFailure(stagedCopy, ex);
        ...
        public virtual void RecoverStagedFiles(SqliteConnection connection)
        {
        ...
        public void DeletePhysicalFileIfUnreferenced(string savedFileName, SqliteConnection connection)
        {
            ...
            {
                PendingFileOperationQueue.RecordCleanupFailure(savedFileName, filePath, ex);
        ...
        private static void EnsureSupportedFileSize(string sourcePath)
        {
            long length = new FileInfo(sourcePath).Length;
            if (length > MaxAttachmentFileSizeBytes)
            {
                throw new InvalidOperationException(
                    $"حجم المرفق {Path.GetFileName(sourcePath)} يتجاوز الحد الأقصى المسموح به وهو 25 ميجابايت.");
            }
        }

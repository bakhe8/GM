...
3. **`Guarantees`**
   ...
   - وأُغلقت الآن خطوة تاسعة لتعريف دور اللوحة الجانبية نفسها:
     ...
     - وبقيت كمساحة مفتوحة دائمًا للوصول السريع إلى:
       ...
       - تاريخه
       - مرفقاته
       - وأزرار الإجراء السريع مثل التمديد والإفراج
   ...
   - وأُغلقت الآن خطوة عاشرة لإزالة التداخل من الجهة الأخرى:
     - ما يظهر في اللوحة الجانبية لم يعد يظهر داخل نافذة `ملف الضمان`
     - أُزيلت من النافذة المنفصلة:
       - بيانات الضمان التفصيلية
       - الخط الزمني
       - المرفقات
       - شريط الأفعال السريعة
     ...
     - أي تركيز قديم يصل إلى هذه الأقسام من handoff سابق يُحوّل الآن إلى أقرب قسم عميق داخل الملف:
       - `Actions` إلى بداية القرار
       - `Series` إلى الطلبات
       - `Attachments` إلى المخرجات
4. **بقايا `Guarantee File`**
   - بعد الفصل الأخير لم تعد بقايا الملف تشمل `Actions Footer` أو `Timeline` أو `Attachments` داخل النافذة المنفصلة
...
5. **العائلة التابعة**
   ...
   - تحقق حي:
     ...
     - وظهرت منطقة `الإجراءات` مباشرة مع أزرار مثل `إنشاء طلب تمديد` و`فتح طلبات الضمان`

...
- ثم أُغلق الآن الحكم المعماري على `Notifications` نفسها:
  ...
  - لكننا لم نخفضها من السايدبار فعليًا بعد
  - والسبب أن `اليوم` رغم توسعها لم تحمل بعد:
    - metrics متابعة أكثر استقلالًا
    - وdetail تكفي وحدها لإخراج `Notifications` من السايدبار تمامًا
  - لذلك صارت `Notifications` تُعامل الآن كالتالي:
    - **عدسة انتقالية تفصيلية**
    - لا **بيتًا علويًا مستقلًا طويل الأمد**
...
- وبدأ الآن **أول تنفيذ معماري مباشر على السايدبار نفسها**:
  - ترتيب `العمل اليومي` صار:
    ...
    - `الطلبات`
    - `الضمانات`
  - `التنبيهات` لم تعد تقرأ كعنصر مساوٍ بلا سياق، بل تحت:
    - `متابعة اليوم`
...
- ثم أُغلقت جولة `Reports/Exports` من نفس العائلة، وخرجت بإصلاحين واضحين في سطح `Reports`:
  - `إنشاء التقرير` لم يعد silent؛ صار يعلن النجاح عبر الشريط السفلي:
    ...
    - `التقارير • المحفظة الكاملة`
  - `فتح التقرير` لم يعد silent أيضًا؛ صار يعلن:
    - `تم فتح التقرير.`
    - `التقارير • Bank_Guarantees_20260428_1311.xlsx`

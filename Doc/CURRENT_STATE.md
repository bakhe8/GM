...
4. **بقايا `Guarantee File`**
   ...
   - هذه صقل متبقٍ فقط، لا إعادة تعريف جديدة للملف
   - وأُغلقت الآن أول نقطة من هذه البقايا:
     - مراسي أقسام الملف أصبحت قابلة للتركيز عند الانتقال إليها من `خريطة الملف`
     - الأقسام القابلة للتمرير ما زالت تنتقل إلى موضعها داخل `ScrollViewer`
     - `Actions Footer` صار يعامل كجزء ثابت خارج التمرير، فيُركز مباشرة بدل محاولة حساب موضع غير تابع لمحتوى التمرير
   - تحقق حي:
     - `الخط الزمني` نقل زر `انتقل إلى الطلبات الكاملة` من أسفل النافذة إلى موضع مرئي داخل ملف `BG-2026-0007`
     - `المرفقات` نقل زر `عرض جميع المرفقات` إلى موضع مرئي
     - `الإجراءات` أبقى شريط الأفعال ظاهرًا، مع ظهور `تسجيل رد البنك` و`فتح طلبات الضمان`

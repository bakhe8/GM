    ...
    public partial class GuaranteeFileView : UserControl, IGuaranteeFileWorkspace
    {
        ...
        private void UpdateExtensionSection()
        {
            ...
            TxtExtensionHint.Text = hasPendingExtension
                ? "يوجد طلب تمديد معلق لهذا الضمان. يمكنك فتحه من قائمة الطلبات المرتبطة أدناه أو من شاشة الطلبات."
                : $"الضمان نشط وينتهي في {_currentGuarantee.ExpiryDate:yyyy-MM-dd}. يمكنك بدء طلب تمديد جديد من نفس السياق هنا.";
        }
        ...
        private void RequestExtension_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                AppDialogService.ShowWarning("يوجد بالفعل طلب تمديد معلق لهذا الضمان. يمكنك فتحه من الطلبات المرتبطة في هذا الملف.");
            ...
            shell.ShowSideSheet(
                "بدء طلب تمديد",
                "إنشاء طلب معلق من نفس سياق ملف الضمان دون مغادرة الصفحة.",
                new CreateExtensionRequestSideSheetView(
                    guarantee,
                    _databaseService,
                    _workflowService,
                    HandleExtensionRequestCreated,
                    FocusExistingRequestFromSideSheet));
        }

        private void HandleExtensionRequestCreated(WorkflowRequest request)
        {
            Guarantee? guaranteeToRefresh = _currentGuarantee;
            if (guaranteeToRefresh == null)
            {
                guaranteeToRefresh = _databaseService.GetCurrentGuaranteeByRootId(request.RootGuaranteeId);
            }
            ...
            LoadGuarantee(guaranteeToRefresh, userInitiated: false);
            FocusSection(GuaranteeFileFocusArea.Actions);
        }

        private void FocusExistingRequestFromSideSheet(int requestId)
        {
            if (_currentGuarantee == null)
            {
                return;

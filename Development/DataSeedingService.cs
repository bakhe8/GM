    ...
    public class DataSeedingService
    {
        ...
        public void Seed(bool clearExistingData = true)
        {
            try
            {
                ...
                // ── مفرج عنها ──────────────────────────────────────────────────────
                SeedReleased_Simple();
                SeedReleased_WithPendingAnnulment();
                SeedReleased_ThenClosed();
                SeedReleased_AnnulmentRejected();
                SeedReleased_AnnulmentCancelled();

                // ── مسيّلة ──────────────────────────────────────────────────────────
                SeedLiquidated_Simple();
                SeedLiquidated_ThenClosed();
                SeedLiquidated_AnnulmentRejected();
        ...
        private void SeedReleased_WithPendingAnnulment()
        {
            var g = BuildGuarantee(0, -30, 500_000m, GuaranteeReferenceType.PurchaseOrder, "PO-7799");
            var current = Save(g, attachmentCount: 0);

            var relReq = _workflowService.CreateReleaseRequest(current.Id, "إفراج بعد انتهاء أمر الشراء", "لمياء العسيري");
            _workflowService.RecordBankResponse(relReq.Id, RequestStatus.Executed, "إفراج مكتمل");

            current = GetCurrent(current)!;
            _workflowService.CreateAnnulmentRequest(current.Id, "طلب نقض الضمان المفرج عنه", "لمياء العسيري");
        }

        private void SeedReleased_ThenClosed()
        {
            var g = BuildGuarantee(1, -45, 1_000_000m, GuaranteeReferenceType.Contract, "عقد-8810");
            var current = Save(g, attachmentCount: 1);

            var relReq = _workflowService.CreateReleaseRequest(current.Id, "إفراج نهائي", "بدر الخالدي");
            _workflowService.RecordBankResponse(relReq.Id, RequestStatus.Executed, "تم الإفراج");
        ...
        {
            var g = BuildGuarantee(2, -20, 650_000m, GuaranteeReferenceType.Contract, "عقد-9200");
            var current = Save(g, attachmentCount: 0);

            var relReq = _workflowService.CreateReleaseRequest(current.Id, "إفراج روتيني", "حمد الطويل");
            _workflowService.RecordBankResponse(relReq.Id, RequestStatus.Executed, "تم الإفراج");

            current = GetCurrent(current)!;
            var annReq = _workflowService.CreateAnnulmentRequest(current.Id, "طلب نقض بعد الإفراج", "حمد الطويل");
            _workflowService.RecordBankResponse(annReq.Id, RequestStatus.Rejected, "رفض البنك النقض لوجود التزامات معلقة");
            // الضمان يبقى في حالة مفرج بعد رفض النقض
        }

        private void SeedReleased_AnnulmentCancelled()
        {
            var g = BuildGuarantee(3, -35, 430_000m, GuaranteeReferenceType.PurchaseOrder, "PO-9300");
            var current = Save(g, attachmentCount: 1);

            var relReq = _workflowService.CreateReleaseRequest(current.Id, "إفراج كامل", "غادة الزهراني");
            _workflowService.RecordBankResponse(relReq.Id, RequestStatus.Executed, "إفراج منتهٍ");

            current = GetCurrent(current)!;
            var annReq = _workflowService.CreateAnnulmentRequest(current.Id, "طلب نقض تم إلغاؤه", "غادة الزهراني");
            _workflowService.RecordBankResponse(annReq.Id, RequestStatus.Cancelled, "تم إلغاء طلب النقض بناءً على قرار إداري");
            // الضمان يبقى في حالة مفرج بعد إلغاء النقض
        }
        ...
        private void SeedLiquidated_ThenClosed()
        {
            var g = BuildGuarantee(5, -50, 2_200_000m, GuaranteeReferenceType.PurchaseOrder, "PO-1033");
            var current = Save(g, attachmentCount: 1);

            var liqReq = _workflowService.CreateLiquidationRequest(current.Id, "تسييل بسبب عدم الاستلام", "نادية الفيفي");
            _workflowService.RecordBankResponse(liqReq.Id, RequestStatus.Executed, "تسييل مكتمل");
        ...
        {
            var g = BuildGuarantee(6, -25, 980_000m, GuaranteeReferenceType.Contract, "عقد-9400");
            var current = Save(g, attachmentCount: 0);

            var liqReq = _workflowService.CreateLiquidationRequest(current.Id, "تسييل بسبب إخلال تعاقدي", "طارق العنزي");
            _workflowService.RecordBankResponse(liqReq.Id, RequestStatus.Executed, "تسييل مكتمل");

            current = GetCurrent(current)!;
            var annReq = _workflowService.CreateAnnulmentRequest(current.Id, "طلب نقض الضمان المسيّل", "طارق العنزي");
            _workflowService.RecordBankResponse(annReq.Id, RequestStatus.Rejected, "رفض البنك النقض لأن إجراءات التسييل لم تكتمل رسمياً");
            // الضمان يبقى في حالة مسيّل بعد رفض النقض
        }
        ...
        {
            List<string> files = CreateDummyFiles(attachmentCount);
            _databaseService.SaveGuarantee(g, files);
        ...
        private List<string> CreateDummyFiles(int count)
        {
